...
from GJ516X.utils.database import (delete_authuser, get_authuser,
                                       get_authuser_names,
                                       save_authuser)
from GJ516X.utils.decorators import AdminActual
from GJ516X.utils.formatters import int_to_alpha
    ...
    if not message.reply_to_message:
        ...
        if get:
            if user.id in get:
                get.remove(user.id)
        if deleted:
            await message.reply_sticker("CAACAgUAAx0CZp7eIAACaD5jT_BrAvAwadEBDpsd7GVSFeYXPAACdAYAArJscVY0KNBsR3kYDioE")
